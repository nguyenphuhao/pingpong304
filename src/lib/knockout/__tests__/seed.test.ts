...
describe("buildDoublesBracket", () => {
  ...
  test("creates 7 matches", () => {
    expect(buildDoublesBracket(seeds)).toHaveLength(7);
  });

  test("QF cross-group: A1 vs D2, C1 vs B2, B1 vs C2, D1 vs A2", () => {
    const qf = buildDoublesBracket(seeds).filter((m) => m.round === "qf");
  ...
  test("next_match_id links QF→SF→F", () => {
    const byId = new Map(buildDoublesBracket(seeds).map((m) => [m.id, m]));
  ...
  test("all matches best_of 5", () => {
    for (const m of buildDoublesBracket(seeds)) expect(m.best_of).toBe(5);
  });
});
...
describe("buildTeamBracket", () => {
  ...
  test("creates 3 matches", () => {
    expect(buildTeamBracket(seeds)).toHaveLength(3);
  });

  test("SF cross-group: A1 vs B2, B1 vs A2", () => {
    const sf = buildTeamBracket(seeds).filter((m) => m.round === "sf");
  ...
  test("each match has 3 individual sub-matches", () => {
    for (const m of buildTeamBracket(seeds)) {
  ...
  test("SF→F links", () => {
    const byId = new Map(buildTeamBracket(seeds).map((m) => [m.id, m]));
